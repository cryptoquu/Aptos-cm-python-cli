...
def offer():
    ...
    creator = AccountAddress.from_hex('0x4929d39991658b0dc6f8ed1e2064f128538e069385b0da930988dce34c121631')
    recipient = AccountAddress.from_hex('0xc1b8b1257eaa926c91aabb48f52851f4d9d05d93beffc10d1130729b548008eb')
    with open(os.path.join(sys.path[0], "config.json"), 'r') as f:
            config = json.load(f)
    ...
    txn_hash = rest_client.offer_token(
        ...
        token_name='Many faces of seppo 15',
        property_version=0,
        amount=1
    )

    rest_client.wait_for_transaction(txn_hash)
    print("\n Offer success, txn hash: " + txn_hash)
...
def printOwnedCount():
    cmAccount = get_cm_account()
    res = rest_client.account_resource(cmAccount.address(), '0x3::token::TokenStore')

    deposit_events = rest_client.get_events_by_handle(cmAccount.address(), '0x3::token::TokenStore', 'deposit_events')
    withdraw_events = rest_client.get_events_by_handle(cmAccount.address(), '0x3::token::TokenStore', 'withdraw_events')

    print("Wallet token count: " + str(len(deposit_events) - len(withdraw_events)));
